...
export function ApplyForm({ isNigeria }: { isNigeria: boolean }) {
  return (
    <Suspense>
      <ApplyFormInner isNigeria={isNigeria} />
    </Suspense>
  )
}

function ApplyFormInner({ isNigeria }: { isNigeria: boolean }) {
  const searchParams = useSearchParams()
  const paramPlan = searchParams.get('plan')
  const [selectedPlan, setSelectedPlan] = useState<PlanId>(
    paramPlan === 'premium' ? 'premium' : 'basic'
  )
        ...
        <Card className="p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" disabled={submitting} className="w-full">
              {submitting ? 'Redirecting to payment...' : `Proceed to Pay — ${formatPrice(plan, isNigeria)}`}
            </Button>
          </form>
        </Card>
